export function renderStudentJobDetail({ state, actions, param, navigate, toaster }) {
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:18px;">
      <div>
        ...
        <div style="display:flex; gap:12px; flex-wrap:wrap; color: var(--text-light); font-size:13px;">
          ...
          <span>学历要求：${job.education}及以上</span>
          <span>截止：${job.deadline}</span>
        </div>
      </div>
      <div style="display:flex; gap:12px;">
        <button class="button button--outline" data-action="favorite">${job.favorite ? "已收藏" : "收藏职位"}</button>
        <button class="button" data-action="apply">立即投递</button>
      </div>
    </div>
  ...
  timelineCard.innerHTML = `
    <h3 class="section-title">投递流程提醒</h3>
    <div class="timeline">
      <div class="timeline-item">
        <div class="timeline-item__time">${job.publishDate}</div>
        <div class="timeline-item__content">
          <div style="font-weight:500;">岗位发布</div>
          <div style="color: var(--text-light);">官方发布职位信息，开放投递入口。</div>
        </div>
      </div>
      <div class="timeline-item">
        <div class="timeline-item__time">${job.deadline}</div>
        <div class="timeline-item__content">
          <div style="font-weight:500;">投递截止</div>
          <div style="color: var(--text-light);">建议提前准备面试材料，避免高峰期投递失败。</div>
        </div>
      </div>
    </div>
  `;
  ...
  jobCard.addEventListener("click", (event) => {
    ...
    if (action === "apply") {
      ...
      toaster.show("投递成功，已同步到我的投递", { type: "success" });
    }
  });
